...
public:    
    ...
    {
        if (auto it = properties.find(AbilityType::id); it != properties.end()) {
    ...
    template <typename AbilityType>
    auto &add(auto&&... _args) {
        auto [_, success] = properties.try_emplace(AbilityType::id, 
...
private:
    std::string name;
    std::unordered_map<std::size_t, Property::Pointer> properties;
};
